            ...
            {formats.map((format) => (
              <Card 
                ...
                className={`bg-gray-900/50 border-white/10 overflow-hidden group ${
                  format.status === 'active' ? 'cursor-pointer hover:border-' + format.color + '-500/50' : 'opacity-75'
                } transition-all`}
              ...
              >
                <div className="relative h-64 overflow-hidden">
                  ...
                  <div className={`absolute bottom-4 left-4 w-14 h-14 bg-${format.color}-500/20 rounded-full flex items-center justify-center border-2 border-${format.color}-500/50`}>
                    <Icon name={format.icon as any} size={28} className={`text-${format.color}-400`} />
                  </div>
                </div>
                ...
                <CardContent className="p-6">
                  ...
                  {format.status === 'active' ? (
                    <Button 
                      className={`w-full bg-${format.color}-500 hover:bg-${format.color}-600 text-black font-semibold`}
                      onClick={(e) => {
                        e.stopPropagation();
